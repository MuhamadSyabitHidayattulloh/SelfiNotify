...
class SocketService {
        ...
        this.io.on('connection', (socket) => {
            ...
            socket.on('authenticate', async (data) => {
                try {
                    ...
                    const application = await ApplicationModel.findByToken(app_token);
                    if (!application) {
                        socket.emit('auth_error', {
                    ...
                    console.log(`Client ${socket.id} authenticated for app: ${application.name}`);
    ...
    sendConnectionStats(socket) {
        ...
        socket.emit('connection_stats', {
            total_connections: Array.from(this.connectedClients.values())
